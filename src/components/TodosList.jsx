...
const TodosList = ({ todos, setTodos, sortType }) => {
  ...
  const sortedTodo = useMemo(() => {
    if (sortType === "Date") {
      ...
      const pineTodo = todos.filter((todo) => todo.pin === true);
      return pineTodo ? pineTodo : [];
  ...
  const handleUpdates = async (todo) => {
    ...
    try {
      const response = await axiosPrivateInterceptor.patch(
        "/api/v1/todo/update",
        JSON.stringify({ ...todo })
      );
  ...
  const handleDelete = async (_id) => {
    ...
    try {
      await axiosPrivateInterceptor.delete(`/api/v1/todo/${_id}`);
      setTodos([...newTodos]);
    } catch (error) {
      console.error(error);
    }
